...
@ExampleDefinition()
class FreeSurfaceSeries3DEllipsoid : SingleChart3DFragment() {
    override fun initExample(surface: SciChartSurface3D) {
        createEllipsoid3DChart()
    }

    fun createEllipsoid3DChart() {
        // <CreateEllipsoid3DChart>
        val sizeU = 40
        val sizeV = 20
        val meshDataSeries = EllipsoidDataSeries3D(
            Double::class.java, sizeU, sizeV
        )
        meshDataSeries.a = 6.0
        meshDataSeries.b = 6.0
        meshDataSeries.c = 6.0
        val random = Random()
